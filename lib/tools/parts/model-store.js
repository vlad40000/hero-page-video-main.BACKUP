...
function toModelStorePayload(row) {
  return {
    ...
    providerEvidences: Array.isArray(row.provider_evidences) ? row.provider_evidences : [],
    completeness: {
  ...
  try {
    const rows = await sql`
      SELECT 
        ...
        fallback_sources,
        provider_plan_json,
        provider_evidences_json as provider_evidences,
        conflict_flags,
        updated_at
  ...
  try {
    await sql`
      ...
      VALUES (
        ...
        ${JSON.stringify(providerEvidences || [])}::jsonb,
      ...
      ON CONFLICT (normalized_model) DO UPDATE SET
        ...
        provider_evidences_json = EXCLUDED.provider_evidences_json,

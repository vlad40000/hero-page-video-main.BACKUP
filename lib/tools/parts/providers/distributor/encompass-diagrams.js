...
function extractRowPrice($row) {
  const priceText = cleanText(
    $row.find('.list-price, .sale-price, .price, [class*="ListPrice"], [class*="SalePrice"], [class*="Price"], td.list-price, td.price').first().text()
  );
...
export function parseEncompassModelPage(html, modelNumber, sourceUrl) {
  ...
  $(
    ...
    $section.find('tr.part-row, tbody tr, tr[data-part-id]').each((_j, rowEl) => {
      ...
      const { retailPrice, retailPriceText } = extractRowPrice($row);

...
/**
 * Canonical model parts URL:
 *   https://encompass.com/service/Parts/{Brand}/{ModelNumber}/
 */
function buildModelPartsUrl(brand, modelNumber) {
  const safeBrand = encodeURIComponent(String(brand || 'Appliance').replace(/\s+/g, '-'));
  return `${BASE_URL}/service/Parts/${safeBrand}/${encodeURIComponent(modelNumber.toUpperCase())}/`;
}
  ...
  $(
    ...
    $section.find('tr.part-row, tbody tr, tr[data-part-id]').each((_j, rowEl) => {
      ...
      parts.push({
        ...
        diagramRef,
        source: DOMAIN,
        sourceUrl: sourceUrl || buildModelPartsUrl('Appliance', modelNumber),
      });
    });
  });
...
export function parseEncompassSearchResults(html, modelNumber) {
  ...
  const candidates = [];
  $('a[href*="/service/Parts/"]').each((_i, el) => {
...
export async function fetchEncompassDistributorBom({ modelNumber, brand, plan = {} }) {
  ...
  const directUrl = buildModelPartsUrl(brand || 'Appliance', modelNumber);
  ...
  try {
    ...
    // If 404, try search-based resolution
    if (Number(err?.status) === 404) {
      try {
        const searchHtml = await fetchHtml(buildSearchUrl(modelNumber));
        ...
        return _blockedResult(modelNumber, plan, searchErr?.url || buildSearchUrl(modelNumber), searchAvailability);
      }
    } else {
      return _blockedResult(modelNumber, plan, err?.url || directUrl, { availability, reason });
    }
  }
  ...
  if (!html) {
    return {
      ...
      parts: [],
      sources: [{ title: 'Encompass Parts', uri: directUrl }],
      coverage: {
        provider: DOMAIN,

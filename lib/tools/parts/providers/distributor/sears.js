...
async function fetchDeterministicSearsBom({ modelNumber }) {
  let modelUrl = null;
  try {
    modelUrl = await resolveSearsModelDetailUrl({ modelNumber });
  } catch (error) {
    const availability = availabilityFromError(error);
    return {
      summary: `Sears PartsDirect request failed for ${modelNumber}.`,
      source: 'searspartsdirect.com',
      rows: [],
      sources: [{ title: 'Sears PartsDirect search', uri: buildSearchUrl(modelNumber) }],
      expectedPartCount: null,
      loadedPartCount: 0,
      retrievalState: availability.availability,
      flags: [availability.availability === 'blocked_403' ? 'provider-blocked-403' : 'provider-fetch-failed'],
      providerAttempts: [
        buildProviderAttempt({
          ...
          sourceUrl: error?.url || buildSearchUrl(modelNumber),
        }),
      ],
    };
  }
    ...
    return {
      ...
      providerAttempts: [
        buildProviderAttempt({
          ...
          reason: 'Model resolver found no exact Sears model detail URL.',
          sourceUrl: buildSearchUrl(modelNumber),
        }),
      ],
    };
  }
  ...
  let first;
  try {
    first = await parseSearsUrl(modelUrl, modelNumber);
  } catch (error) {
    const availability = availabilityFromError(error);
    return {
      summary: `Sears PartsDirect model detail request failed for ${modelNumber}.`,
      source: 'searspartsdirect.com',
      rows: [],
      ...
      providerAttemptFromResult(
        'searspartsdirect.com',
        {
          ...
          coverage: {
            flags: retrievalState === 'parts_partial' ? ['sears-parts-partial'] : [],
            retrievalState,
            sectionsDiscovered: new Set(rows.map((row) => row.sectionName).filter(Boolean)).size,
          },
        },
...
export async function fetchSearsDistributorBom({ modelNumber, brand, plan = {} }) {
  ...
  if (deterministic.rows.length > 0 || process.env.ENABLE_GEMINI_SEARCH_FALLBACK !== 'true') {
    ...
    return {
      ...
      providerAttempts: deterministic.providerAttempts || [
        providerAttemptFromResult('searspartsdirect.com', {
          parts: deterministic.rows,
          sources: deterministic.sources,
          coverage: { flags: deterministic.flags, retrievalState: deterministic.retrievalState },
        }, 'distributor_fallback'),
      ],
      planMeta: {

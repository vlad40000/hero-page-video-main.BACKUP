...
async function resolveDlPartsModelUrl(modelNumber) {
  // DB-first: use a known seeded URL when available
  const dbUrl = await resolveDlPartsUrlFromDb(modelNumber);
  if (dbUrl) {
    const html = await fetchHtml(dbUrl);
    const parsed = parseDlPartsModelPage(html, modelNumber, dbUrl);
    return { url: dbUrl, html, parsed };
  }

  // Direct model page; fall back to search on 404

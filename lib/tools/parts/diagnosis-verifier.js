...
export async function verifyDiagnosticResultsWithPartsContext(results = [], partsContext = {}, options = {}) {
  ...
  for (const row of verificationRows) {
    ...
    if (!hasVerifiedPrice(snapshot)) {
      continue;
    }

    const verificationStatus = match ? 'model_part_verified' : fallbackCandidate ? 'targeted_part_search_verified' : 'unverified';
    const warnings = [];
    if (!match && !fallbackCandidate) warnings.push('Candidate part was not found in model-specific parts context.');
    if (priceBudgetSkipped) warnings.push('Price verification budget reached; candidate withheld from priced estimate.');
    ...
    verified.push({
      ...
      partNumber: verifiedPartNumber || '',
      partPrice: centsRounded(snapshot?.retailPrice),
      ...
      partPriceSource: snapshot?.retailPriceSource || null,
      partPriceVerified: Boolean(snapshot?.retailPriceVerified),
      partPricingUrl: snapshot?.retailPricingUrl || null,
      partAvailability: snapshot?.retailAvailability || null,
      contextWarnings: warnings,
      provenance: [
        ...(partsContext.provenance || []),
        ...(fallbackCandidate?.provenance || []),
        ...(snapshot ? [{ source: snapshot.retailPriceSource, uri: snapshot.retailPricingUrl, note: 'Internal supplier catalog price verification.' }] : []),
      ],
    });
  }

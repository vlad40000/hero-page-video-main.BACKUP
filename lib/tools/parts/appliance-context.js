...
function priceFromProviderRows(providerRows = []) {
  for (const row of providerRows) {
    ...
    return {
      retailPrice: price,
      retailPriceText: row.retailPriceText || payload.retailPriceText || payload.retail_price_text || `$${price.toFixed(2)}`,
      retailAvailability: normalizeAvailabilityLabel(row.retailAvailability || row.availabilityStatus || payload.availabilityStatus),
      retailPricingUrl: row.evidenceUrl || row.sectionUrl || row.evidence_url || row.section_url || rawPayload.evidenceUrl || rawPayload.sectionUrl || payload.sourceUrl || null,
      retailPriceSource: row.source || payload.source || 'supplier_catalog',

...
export async function startApplianceSearchSession({
  ...
  await upsertModelToStore({ normalizedModel, rawModel: modelNumber, payload }).catch(e => console.error('staged cache fail', e));
...
export async function continueApplianceSearchSession({ searchSessionId, revision = null }) {
  ...
  await upsertModelToStore({ normalizedModel: norm, rawModel: session.request.modelNumber, payload }).catch(e => console.error('staged cache fail', e));

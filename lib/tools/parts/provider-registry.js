...
import { getProviderCapability } from '@/lib/tools/parts/provider-availability';
...
export function buildProviderPlan({ modelNumber, brand, exhaustiveMode = false }) {
  ...
  const fastPrimaryMax = toPositiveInt(process.env.MAX_PRIMARY_FALLBACK_DOMAINS, 3);
  const fastSecondaryMax = toPositiveInt(process.env.MAX_SECONDARY_FALLBACK_DOMAINS, 2);
  ...
  return {
    ...
    providerAvailability: Object.fromEntries(
      distributorFallbacks.map((domain) => [domain, getProviderCapability(domain)])
    ),
